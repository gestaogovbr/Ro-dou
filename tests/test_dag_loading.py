...
class TestDAGLoading:
    ...
    def test_all_dags_load_without_errors(self, mock_get_connection, mock_variable):
        ...
        try:
            ...
            assert (
                dag_count > 0
            ), "Nenhuma DAG foi gerada a partir dos arquivos de configuração YAML"

        finally:
            for name in list(globals().keys()):
                if name not in original_globals and isinstance(
                    globals().get(name), DAG
                ):
                    del globals()[name]
    ...
    def test_dag_generation_handles_missing_variables(self, mock_variable):
        ...
        for directory in self.dag_generator.YAMLS_DIR_LIST:
            if os.path.exists(directory):
                for dirpath, _, filenames in os.walk(directory):
                    for filename in filenames:
                        if "variable" in filename.lower() and filename.endswith(
                            (".yaml", ".yml")
                        ):
        ...
        if variable_yaml_files:
            for yaml_file in variable_yaml_files[:1]:
                try:
                    ...
                    assert (
                        "Não encontrado" in str(e).lower()
                        or "variável" in str(e).lower()
                    )
        else:
            pytest.skip(
                "Nenhum arquivo YAML baseado em variáveis encontrado para teste"
            )
