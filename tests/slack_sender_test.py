...
import pytest
import requests
from dags.ro_dou_src.notification.slack_sender import SlackSender, _format_date
from dags.ro_dou_src.notification.isender import remove_html_tags
...
class TestSlackSenderSendMethod:
    def test_send_with_complete_report(
        self, mock_report_config, sample_search_report, mocker: MockerFixture
    ):
        mock_post = mocker.patch("requests.post")
        mock_response = MagicMock()
        mock_post.return_value = mock_response
    ...
    def test_send_with_hidden_filters(
        self, mock_report_config_no_texts, sample_search_report, mocker: MockerFixture
    ):
        mock_post = mocker.patch("requests.post")
        mock_response = MagicMock()
        mock_post.return_value = mock_response
    ...
    def test_send_with_header_and_footer(
        self, mock_report_config, mocker: MockerFixture
    ):
        mock_post = mocker.patch("requests.post")
        mock_response = MagicMock()
        mock_post.return_value = mock_response

        # Mock remove_html_tags to avoid the self parameter issue
        mocker.patch(
            "dags.ro_dou_src.notification.isender.remove_html_tags",
