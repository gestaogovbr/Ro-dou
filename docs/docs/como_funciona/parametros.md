...
- **ignore_inline_tables** *(opcional)*: Define se as tabelas HTML embutidas no corpo da própria publicação devem ser substituídas por um marcador de texto. Diferente de `ignore_attachments` (que remove registros inteiros do banco), este parâmetro atua sobre tabelas que fazem parte do texto principal — por exemplo, uma portaria que lista dezenas de beneficiários numa tabela dentro do próprio ato. O marcador exibido tem o formato `[Tabela de N linhas omitida]`. Aplica-se tanto ao recorte quanto ao `full_text: True`. Valores: True ou False. Default: False. (Funcionalidade disponível apenas no INLABS)
- **min_table_rows** *(opcional)*: Parâmetro complementar a `ignore_inline_tables` — só produz efeito quando `ignore_inline_tables: True`; se `ignore_inline_tables` não estiver habilitado (ou estiver `False`), `min_table_rows` é ignorado e nenhuma tabela é omitida, independente do valor configurado. Define o número mínimo de linhas que uma tabela deve ter para ser omitida. Com o default `1`, todas as tabelas são omitidas. Defina um valor maior para manter tabelas menores no texto (que passam a ser exceção à regra), pois provavelmente carregam informações estruturais do ato (ex: quadro de vigência, tabela de valores) — por exemplo, `min_table_rows: 5` omite apenas tabelas com 5 ou mais linhas. Default: 1. (Funcionalidade disponível apenas no INLABS)
