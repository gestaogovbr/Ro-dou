...
- **min_table_rows** *(opcional)*: Número mínimo de linhas que uma tabela deve ter para ser omitida quando `ignore_inline_tables: True`. Com o default `1`, todas as tabelas são omitidas. Defina um valor maior para manter tabelas menores no texto (que passam a ser exceção à regra), pois provavelmente carregam informações estruturais do ato (ex: quadro de vigência, tabela de valores) — por exemplo, `min_table_rows: 5` omite apenas tabelas com 5 ou mais linhas. Default: 1. (Funcionalidade disponível apenas no INLABS)
