...
- **ignore_inline_tables** *(opcional)*: Define se as tabelas HTML embutidas no corpo da própria publicação devem ser substituídas por um marcador de texto no recorte. Diferente de `ignore_attachments` (que remove registros inteiros do banco), este parâmetro atua sobre tabelas que fazem parte do texto principal — por exemplo, uma portaria que lista dezenas de beneficiários numa tabela dentro do próprio ato. O marcador exibido tem o formato `[Tabela de N linhas omitida]`. Não tem efeito quando `full_text: True`. Valores: True ou False. Default: False. (Funcionalidade disponível apenas no INLABS)
- **min_table_rows** *(opcional)*: Número mínimo de linhas que uma tabela deve ter para ser omitida quando `ignore_inline_tables: True`. Tabelas com menos linhas do que esse valor são mantidas no recorte, pois provavelmente carregam informações estruturais do ato (ex: quadro de vigência, tabela de valores). Default: 3. (Funcionalidade disponível apenas no INLABS)
