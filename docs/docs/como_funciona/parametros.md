...
- **active** *(opcional)*: Se `True`, a DAG é criada e já fica ativa (unpaused) no Airflow. Se `False` (padrão), a DAG é criada mas permanece pausada, não sendo executada automaticamente até ser ativada manualmente na interface do Airflow.
