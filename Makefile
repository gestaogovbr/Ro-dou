...
.PHONY: build-images
build-images:
	$(COMPOSE) -p ro-dou build airflow-webserver airflow-scheduler

create-logs-dir:
	mkdir -p ./mnt/airflow-logs -m a=rwx

setup-containers:
	$(COMPOSE) -p ro-dou up -d
...
redeploy: build-images
	$(COMPOSE) -p ro-dou up -d --no-deps airflow-webserver airflow-scheduler
	$(MAKE) smoke-test

.PHONY: down
down:
	$(COMPOSE) -p ro-dou down
