...
create-example-variable:
	...
	@docker exec airflow-api-server sh -c 		"if ! curl -f -s -LI 'http://localhost:8080/api/v2/variables/termos_exemplo_variavel' --user \"airflow:airflow\" > /dev/null; \
		then \
			curl -s -X 'POST' \
...
create-email-admim-variable:
	...
	@docker exec airflow-api-server sh -c \
